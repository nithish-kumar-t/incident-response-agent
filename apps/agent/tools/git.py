...
# ── Git-based reads (for when the service uses a remote/versioned repo) ───────
...
# ── Filesystem-based reads (direct repo clone on disk) ────────────────────────

def read_file_lines(repo_path: str, file_path: str, start_line: int, end_line: int) -> str:
    """
    Read a specific range of lines from a file in the repo.
    This is the key tool for pinpointing an exact error location from a stack trace.
    """
...
def grep_codebase(
    ...
    """
    Search the codebase for a regex pattern. Returns file:line matches.
    Use this to trace an exception class, function name, or variable from the
    stack trace directly to its definition in the source code.
    """
    ...
    for fpath in root.rglob("*"):
        ...
        if ext_set and fpath.suffix.lstrip(".").lower() not in ext_set:
            continue
        # Skip binary-ish and hidden files
...
def parse_stack_trace_locations(stack_trace: str) -> list[dict]:
    ...
    Handles Python (File "x.py", line N), Java (at pkg.Class(File.java:N)),
    Go (goroutine / file.go:N), and generic patterns.
    """
    locations: list[dict] = []
    seen: set[tuple] = set()
    ...
    patterns = [
        ...
        # Java: at com.example.Class(FileName.java:42)
