...
Uses the log paths resolved by Agent 1 (not hardcoded config paths).

service_error path:
  1. check_service_health on the resolved health URL
  2. grep_log_file on the resolved service log for the error pattern
  3. extract_stack_trace to capture the full stack trace block
  4. grep_log_file on the resolved error log for corroborating evidence

api_error path:
  1. grep_log_file on the resolved error log
  2. grep_log_file on the resolved api log
  3. extract_stack_trace from whichever log had the hit

Output carries the raw stack trace so Agent 3 can parse file:line locations.
"""
import asyncio
import json
import logging
...
_SYSTEM_SERVICE = """You are the Service Investigation Agent (Agent 2) for a SERVICE ERROR.

You have received the resolved log paths and health URL for this specific service from Agent 1.
...
_SYSTEM_API = """You are the API Investigation Agent (Agent 2) for an API ERROR.

You have the resolved log paths for this specific service from Agent 1. Use them directly.
...
async def _service_executor(fn_name: str, fn_args: dict):
    if fn_name == "check_service_health":
        # health tool expects service_name; we derive it from the URL here
        import httpx
...
async def run(agent1_result: dict) -> dict:
    error_type = agent1_result.get("error_type", "service_error")
    context = json.dumps(agent1_result, indent=2)

    base_msg = (
        f"Agent 1 triage result (includes resolved_config with exact log paths):\n{context}\n\n"
        "Use the log paths from resolved_config['log_paths'] — do not substitute or guess paths."
    )

    if error_type == "service_error":
        raw = await run_agent(_SYSTEM_SERVICE, base_msg, _SERVICE_TOOLS, _service_executor)
    else:
        raw = await run_agent(_SYSTEM_API, base_msg, _SHARED_TOOLS, _api_executor)
