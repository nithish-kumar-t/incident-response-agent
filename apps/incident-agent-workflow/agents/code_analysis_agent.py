"""
Code Analysis Agent — reads exact source files from the stack trace locations
identified by Investigation Agent, then pinpoints the root cause in the code.
Falls back to alternative investigation strategies when scope is too broad.
"""
import asyncio
import json
import logging
...
_SYSTEM = """You are the Code Analysis Agent in an automated incident response pipeline.

You have:
- Triage Agent's result (service name, error type, resolved_config with repo_path)
- Investigation Agent's result (inference, raw stack_trace, suggested_files_to_check)
...
STEP 2 — Read exact code locations:
  ...
  ±20-line window around the implicated line.

STEP 3 — Trace deeper if needed (max 2 hops):
  If the root cause is not at the stack trace location itself, call grep_codebase
  to find its definition, then read_file_lines on that location.
...
COMPLEXITY RULE:
  ...
  with concrete, actionable options. Do NOT produce vague guesses.
...
_TOOLS = [
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "stack_trace": {
                        "type": "string",
                        "description": "The raw stack trace text from Investigation Agent's output",
                    }
                },
    ...
    },
    {
        ...
        "function": {
            "name": "get_git_log",
            "description": "Get recent commit history for a file to spot regressions.",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "List files in the repo (use only to discover structure when no stack trace is available).",
            "parameters": {
                "type": "object",
                "properties": {
                    "repo_path": {"type": "string"},
...
async def _executor(fn_name: str, fn_args: dict):
    if fn_name == "parse_stack_trace":
        ...
        logger.info("[CodeAnalysisAgent] parse_stack_trace → %d locations", len(locations))
        return {"locations": locations}

    if fn_name == "read_file_lines":
        repo_path = fn_args.get("repo_path", "")
        if not repo_path:
            return {"error": "repo_path is required"}
        return await asyncio.to_thread(
            read_file_lines,
            repo_path, fn_args.get("file_path", ""),
            fn_args.get("start_line", 1), fn_args.get("end_line", 50),
        )
...
async def run(triage_result: dict, investigation_result: dict) -> dict:
    repo_path = (triage_result.get("resolved_config") or {}).get("repo_path", "unknown")
    stack_trace = investigation_result.get("stack_trace", "")
    suggested = investigation_result.get("suggested_files_to_check", [])

    if stack_trace:
        how_to_start = (
            f"Stack trace is available. Follow STEP 1-4:\n"
        ...
        how_to_start = (
            f"No stack trace available. Do NOT give up — read the code directly:\n"
            f"1. Call list_repo_files to discover structure at {repo_path}.\n"
            f"2. For each file in suggested_files_to_check ({suggested_str}), call "
            f"read_file_lines (lines 1-100, then narrow down).\n"
            ...
            f"before producing your final JSON."
        )

    user_msg = (
        f"Triage Agent result (repo_path = '{repo_path}'):\n{json.dumps(triage_result, indent=2)}\n\n"
